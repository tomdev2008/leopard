		<#include "header.inc.ftl">
		<div class="menu">                
			<!-- 
            <div class="breadLine">            
                <div class="arrow"></div>
                <div class="adminControl active">
                    Hi, 谭海潮
                </div>
            </div>

            <div class="admin">
                <div class="image">
                    <img src="img/users/aqvatarius.jpg" class="img-polaroid"/>                
                </div>
                <ul class="control">                
                    <li><span class="icon-comment"></span> <a href="messages.html">Messages</a> <a href="messages.html" class="caption red">12</a></li>
                    <li><span class="icon-cog"></span> <a href="forms.html">Settings</a></li>
                    <li><span class="icon-share-alt"></span> <a href="login.html">Logout</a></li>
                </ul>
                <div class="info">
                    <span>Welcom back! Your last visit: 24.10.2012 in 19:55</span>
                </div>
            </div>
             -->
             
            <div class="dr"><span></span></div>

            <ul class="navigation">
			    <li class="active"><a href="/monitor/performance.do"><span class="isw-grid"></span><span class="text">方法耗时</span></a></li>
			    <li><a href="/monitor/connection.do"><span class="isw-archive"></span><span class="text">数据源连接数量</span></a></li>
			    <li><a href="/monitor/thread.do"><span class="isw-text_document"></span><span class="text">线程数量</span></a></li>
			    <li><a href="/system/log.do"><span class="isw-text_document"></span><span class="text">日志管理</span></a></li>
			    
			</ul>

            <div class="dr"><span></span></div>

            <div class="widget">

                <div class="input-append">
                    <input id="appendedInputButton" style="width: 118px;" type="text"/><button class="btn" type="button">Search</button>
                </div>            

            </div>

            <div class="dr"><span></span></div>
        </div>